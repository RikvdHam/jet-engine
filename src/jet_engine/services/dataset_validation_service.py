...
from jet_engine.db.models import Dataset, DatasetMapping, Field
...
def validate_dataset(db: Session, dataset_id: str) -> Dict:
    ...
    field_lookup = {f.id: f for f in fields}
    rename_dict = {raw_col: field_lookup[fid].canonical_name for raw_col, fid in mapping.items()}
